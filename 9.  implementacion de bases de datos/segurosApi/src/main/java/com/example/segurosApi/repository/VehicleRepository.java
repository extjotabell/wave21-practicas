...
public interface VehicleRepository extends JpaRepository<Vehiculo, Long> {
    ...
    List<Vehiculo> findAllVehicleOrdenByManufacturing();
    ...
    @Query("SELECT v FROM Vehiculo v " +
            "WHERE v.id IN (SELECT s.vehiculo.id FROM v.siniestros s WHERE s.perdidaEconomica > 10000 ) ")
    List<Vehiculo> findAllPatentAndBrandAndModel();

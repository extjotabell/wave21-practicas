...
public class SaleController {
    ...
    public ResponseEntity<SuccessDTO> updateSale(@PathVariable(value = "number") Integer id, @RequestBody SaleRequestDTO theBody){
        if(saleService.updateSale(id, theBody)){
            return ResponseEntity.ok().body(new SuccessDTO());
        }
        throw new RuntimeException();
    }
    ...
    public ResponseEntity<SuccessDTO> deleteSale(@PathVariable Integer number){
        ...
        return ResponseEntity.ok().body(new SuccessDTO("Se ha eliminado con exito el id "+ number, HttpStatus.OK.value()));
    }

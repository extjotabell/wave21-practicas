...
public class SaleController {
    ...
    public ResponseEntity<SuccessDTO> updateSale(@PathVariable Integer number,@RequestBody SaleRequestDTO saleRequestDTO){
        if (saleService.updateSale(number,saleRequestDTO))
            return ResponseEntity.ok().body(new SuccessDTO("Se actualizó la venta", 201));
        else
            throw new RuntimeException();
    }
    ...
    public ResponseEntity<SuccessDTO> deleteSale(@PathVariable Integer number){
        if(saleService.deleteSale(number))
            return ResponseEntity.ok().body(new SuccessDTO("Se eliminó la venta",201));
        else
            throw new RuntimeException();
    }
...
/*    @GetMapping("/sale")
    public ResponseEntity<SaleListResponseDTO> findAllSales(){
        ...
        return ResponseEntity.ok().body(new SaleListResponseDTO());
    }
   @GetMapping("/sale/{number}")
    public ResponseEntity<SaleResponseDTO> findOneSale(){

        return ResponseEntity.ok().body(new SaleResponseDTO());
    }

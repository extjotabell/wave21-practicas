...
public class EmpleadoServiceImpl implements EmpleadoService{
    ...
    public List<Empleado> getEmpleadoByNombreAndCiudad(String nombre, String orden){
        if(orden.toLowerCase().equals("asc"))
            return empleadoRepository.findEmpleadoByNombre(nombre, Sort.by("nombre").ascending());
        else
            return empleadoRepository.findEmpleadoByNombre(nombre, Sort.by("nombre").descending());

...
* */
    ...
    //INDIVIDUAL US0010
    @Override
    public Post createProductWithPromo(Post post) {
        boolean containsKey = dataUser.containsKey(post.getUserId());
        if(containsKey){
            dataUser.get(post.getUserId()).getPosts().add(post);
        }
        return containsKey ? post : null;
    }

    //INDIVIDUAL US0011
    @Override
    public User getProductsWithPromoByUserId(int userId) {
        User result = new User();
        boolean containsKey = dataUser.containsKey(userId);
        if(containsKey){
            result.setId(userId);
            result.setUsername(dataUser.get(userId).getUsername());
            List<Post> posts = dataUser.get(userId).getPosts().stream()
                    .filter(Post::isHasPromo).toList();
            result.setPosts(posts);
        }
        return containsKey ? result : null;
    }

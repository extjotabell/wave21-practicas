package com.example.be_java_hisp_w21_g1_dominguez.Controller;
...
import com.example.be_java_hisp_w21_g1_dominguez.Service.UserService;
...
public class Controller {
   ...
   */
    ...
    public ResponseEntity<?> newProductInSale(@RequestBody PromoPostDTO promoPostDTO) {
        userService.createPromoPost(promoPostDTO);
        return new ResponseEntity<>("Se ha creado el promo-post!", HttpStatus.OK);
    }

    //US 0011: Obtener la cantidad de productos en promoción de un determinado vendedor

    @GetMapping("/products/promo-post/count")
    public ResponseEntity<?> promoProductsCount(@RequestParam(value = "user_id", required = true) int user_id) {
        PromoPostBySellerDTO latestPromoPosts = userService.listPromoPostsBySeller(user_id);
        return new ResponseEntity<>(latestPromoPosts, HttpStatus.OK);
    }

    //US 0012: OPCIONAL
    @GetMapping("/users/{userId}/followed/products/count")
    public ResponseEntity<?> productsInSaleByFollowedSeller(@PathVariable int userId) {
        CountByFollowedSellerDTO countByFollowedSellerDTO = userService.listCountByFollowedSeller(userId);
        return new ResponseEntity<>(countByFollowedSellerDTO, HttpStatus.OK);
    }

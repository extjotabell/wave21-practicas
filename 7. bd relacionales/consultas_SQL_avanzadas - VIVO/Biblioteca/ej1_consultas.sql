...
SELECT autor.Nombre
FROM autor, libro, libro_autor
WHERE libro_autor.idAutor = autor.idAutor AND libro_autor.idLibro = libro.idLibro
AND libro.titulo = "El Universo: Guía de viaje";

-- 10. ¿Qué libros se prestaron al lector “Filippo Galli”?
SELECT libro.*
FROM libro, prestamo
WHERE prestamo.idLector IN (
	SELECT idLector 
	FROM estudiante 
...
WHERE autor.idAutor = libro_autor.idAutor AND libro.idLibro = libro_autor.idLibro AND autor.Nombre = "J.K. Rowling";

-- 14. Listar títulos de los libros que debían devolverse el 16/07/2021.
SELECT libro.titulo
FROM libro, prestamo
WHERE libro.idLibro = prestamo.idLibro
AND prestamo.fecha_devolucion > "2021-07-16" AND prestamo.fecha_prestamo < "2021-07-16";
